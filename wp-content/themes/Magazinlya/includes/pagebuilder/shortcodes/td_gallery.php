...
function td_gallery_inner(&$td_query, $td_column_number = '') { //$td_query by ref
    global $post;
    ...
    $td_unique_id_gallery = uniqid();

    $buffy .= '<div id="' . $td_unique_id_gallery . '" class="flexslider td_gallery-col-' . $td_column_number . ' td_mod_wrap">';
            $buffy .= '<ul class="slides">';

            if ($td_query->have_posts()) {
                while ($td_query->have_posts()) : $td_query->the_post();
                    $buffy .= td_modules::mod_gallery_render($post, $td_post_count);
                    //current column
    ...
    $buffy .= '</div>'; //close flex


    $buffy .= '
<script>
jQuery("#' . $td_unique_id_gallery . '").flexslider({
    animation: "slide",
    animationLoop: true,
    itemWidth: 213,
    itemMargin: 20,
...
function td_gallery($atts) {
    ...
    $buffy .= td_block_builder::get_block_js($atts, $td_unique_id, $td_query, 'gallery');


    $buffy .= '<div class="td_block_wrap td_block_gallery">';
        ...
        $buffy .= '<div id=' . $td_unique_id . ' class="td_block_inner">';
            ...
            $buffy .= td_gallery_inner($td_query);
        $buffy .= '</div>';

    $buffy .= '</div> <!-- ./block1 -->';
    return $buffy;
}
add_shortcode('td_gallery', 'td_gallery');
