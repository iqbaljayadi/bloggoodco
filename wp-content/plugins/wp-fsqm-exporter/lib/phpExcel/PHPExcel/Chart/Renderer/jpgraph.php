...
class PHPExcel_Chart_Renderer_jpgraph
{
	...
	private static $_colourSet = array(
		'#3992CA', '#EB9633', '#FF4E50',
		'#FF6000', '#FFD700', '#10A0FF',
		'#0077FF', '#EAB501', '#82187C',
		'#C80909', '#EC9500', '#0048FF',
		'#0022FF', '#EA7401', '#901111',
		'#591A1A', '#FFD674', '#00C3FF',
		'#00F3FF', '#FFA100', '#CC2A41',
	);
	...
	private static function _getColor($index) {
		return self::$_colourSet[ $index % count( self::$_colourSet ) ];
	}
	...
	private function _formatPointMarker($seriesPlot,$markerID) {
		...
		$seriesPlot->mark->SetColor(self::_getColor(self::$_plotColour));
		$seriesPlot->mark->SetFillColor(self::_getColor(self::$_plotColour));
		$seriesPlot->SetColor(self::_getColor(self::$_plotColour++));
	...
	private function _renderLegend($chartType = '') {
		...
		if (!is_null($legend)) {
			...
			switch ($legendPosition) {
				case 'r'	:
					...
					if($chartType != 'stockChart') {
						$this->_graph->SetMargin(30, 60, 40, 10);
						$this->_graph->AdjustMarginsForTitles();
					}
					break;
				case 'l'	:
			...
			$this->_graph->legend->SetFillColor('#aaa@0.7');
		} else {
	...
	private function _renderPlotLine($groupID, $filled = false, $combination = false, $dimensions = '2d') {
		...
		for($i = 0; $i < $seriesCount; ++$i) {
			...
			if ($filled) {
				...
				$seriesPlot->SetFillColor(self::_getColor(self::$_plotColour++));
			} else {
	...
	private function _renderPlotBar($groupID, $dimensions = '2d') {
		...
		if (($groupID == 0) && ($rotation == 'bar')) {
			$this->_graph->Set90AndMargin(10, 10, 50, 10);
		}
		$grouping = $this->_chart->getPlotArea()->getPlotGroupByIndex($groupID)->getPlotGrouping();
		...
		for($j = 0; $j < $seriesCount; ++$j) {
			...
			//$seriesPlot->SetColor('black');
			$color_cache[$j] = self::_getColor(self::$_plotColour++);
			//$seriesPlot->SetFillColor($color_cache[$j]);
			//$seriesPlot->SetColor($color_cache[$j]);

			//var_dump(self::$_colourSet[self::$_plotColour]);
			if ($dimensions == '3d') {
			...
			$seriesPlot->SetLegend($dataLabel);
			//$seriesPlot->SetScale(0.3);
			$seriesPlot->SetCenter(0.5);
		...
		$this->_graph->Add($groupPlot);
		$groupPlot->SetCenter(0.5, 0.5);

		for($j = 0; $j < $seriesCount; ++$j) {
			$seriesPlots[$j]->SetFillColor($color_cache[$j]);
			$seriesPlots[$j]->SetColor('white');
		}
		//var_dump($groupPlot);
	...
	private function _renderPlotScatter($groupID,$bubble) {
		...
		for($i = 0; $i < $seriesCount; ++$i) {
			...
			if ($scatterStyle == 'lineMarker') {
				...
				$seriesPlot->link->SetColor(self::_getColor(self::$_plotColour));
			} elseif ($scatterStyle == 'smoothMarker') {
				$spline = new Spline($dataValuesY,$dataValuesX);
				list($splineDataY,$splineDataX) = $spline->Get(count($dataValuesX) * self::$_width / 20);
				$lplot = new LinePlot($splineDataX,$splineDataY);
				$lplot->SetColor(self::_getColor(self::$_plotColour));
	...
	private function _renderPlotStock($groupID) {
		...
		for($j = 0; $j < count($dataValues[count($dataValues) - 1]); $j++) {
			for($i = 1; $i < 5; $i++) {
		...
		$xAxisLabel = $dataValues[0];

		foreach($xAxisLabel as $key => $val) {
			$xAxisLabel[$key] = (string) $val;
		}
		...
		$seriesPlot->SetColor('#8DC6C4', 'white', '#3992CA', '#3992CA');
	...
	private function _renderPieChart($groupCount, $dimensions = '2d', $doughnut = False, $multiplePlots = False) {
		...
		for($groupID = 0; $groupID < $iLimit; ++$groupID) {
			...
			for($j = 0; $j < $jLimit; ++$j) {
				...
				if($seriesPlot instanceof PiePlot) {
					$seriesPlot->SetSize(0.3);
					$seriesPlot->SetCenter(0.3, 0.5);
					$seriesPlot->SetLabelType(PIE_VALUE_ADJPERCENTAGE);
				}
				$seriesPlot->SetSliceColors(self::$_colourSet);
				$seriesPlot->SetColor('white');
			}
		}
	...
	private function _renderStockChart($groupCount) {
		...
		//$this->_renderplotara
		$this->_renderCartesianPlotArea('textlin');
	...
	public function render($outputDestination, $width = 640, $height = 480) {
		self::$_width = $width;
		self::$_height = $height;
        self::$_plotColour = 0;
