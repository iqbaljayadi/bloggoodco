...
class Fb_wpress_activity_feed extends WP_Widget {
	
	//Register widget
	public function __construct() {
		add_action( 'widgets_init', create_function( '', 'register_widget( "Fb_wpress_activity_feed" );' ) );
		parent::__construct(
	 		'Fb_wpress_activity_feed', // Base ID
			'Facebook Activity Feed', // Name
			array( 'description' => 'Facebook activity feed (Facebook WPress plugin)' ) // Args
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		$recommendations = apply_filters( 'widget_title', $instance['recommendations'] );
		...
		$criteria = array('domain'=>$domain, 'width'=>$width, 'height'=>$height, 'colorscheme'=>$colorscheme, 'header'=>$header, 'recommendations'=>$recommendations);
	...
	public function form( $instance ) {
		...
		if ( isset( $instance[ 'recommendations' ] ) ) $recommendations = $instance[ 'recommendations' ];
		else $recommendations='false';
		...
		<div style="margin-bottom:5px;">
			<label><?php _e( 'Recommendations:' ); ?></label>
			<select class="widefat" id="<?php echo $this->get_field_id( 'recommendations' ); ?>" name="<?php echo $this->get_field_name( 'recommendations' ); ?>" ><option></option>
			<?php
			foreach($true_false_tab as $ind=>$value) {
				if($ind==$recommendations) echo '<option value="'.$ind.'" selected>'.$value.'</option>';
				else echo '<option value="'.$ind.'">'.$value.'</option>';
			}
			?>
			</select>
