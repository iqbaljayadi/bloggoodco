		...
		afterThemeLoaded : function(op) {
			return this.each(function() {
				...
				if(self.data('ui-theme') && self.data('ui-theme-id')) {
					...
					methods.loadThemes(links, function() {
						methods.applyUIElements.apply(_self, [op.callback, 'ipt-uif-custom-' + ui_theme_id]);
					}, op.themeCheckTimeout);
				} else {
					//No theme give, fallback to the default
					methods.applyUIElements.apply(_self, [op.callback, 'ipt-uif-custom-none']);
				}
			});
		},

		applyUIElements : function(callback, ui_theme_id) {
			var self = $(this);
