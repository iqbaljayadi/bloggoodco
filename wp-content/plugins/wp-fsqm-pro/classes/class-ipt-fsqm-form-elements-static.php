...
<head>
	...
	<?php wp_head(); ?>
	<style type="text/css">
	@import url("http://fonts.googleapis.com/css?family=Oswald|Roboto:400,700,400italic,700italic");
	/* =Reset
	-------------------------------------------------------------- */
		...
		<div class="ipt_fsqm_user_portal_welcome">
			...
			<div class="ipt_fsqm_up_toolbar">
				...
				<div class="ipt_fsqm_up_meta">
					<h6><?php $ui->print_icon_by_class( 'drawer-2', false ); ?><?php printf( _n( '%d Submission', '%d Submissions', $total_count, 'ipt_fsqm' ), $total_count ); ?></h6>
					<?php if ( $showscore == '1' ) : ?>
...
<div class="ipt_uif_front ipt_uif_common">
	<div class="ipt_uif_mother_wrap ui-widget-content ui-widget-default">
		<div id="ipt_fsqm_submission_data_<?php echo $form->data_id; ?>" class="ipt_uif_column ipt_uif_column_full" style="margin-bottom: 20px;">
			<?php $form->ui->heading( __( 'Submission Data', 'ipt_fsqm' ), 'h2', 'left', 0xe020 ); ?>
		...
		<div class="ipt_uif_column ipt_uif_column_full" style="margin-bottom: 20px;">
			...
			<?php $form->ui->heading( __( 'Score & Summary', 'ipt_fsqm' ), 'h2', 'left', 0xe0d3 ); ?>
			<?php else : ?>
			<?php $form->ui->heading( __( 'Print & Summary', 'ipt_fsqm' ), 'h2', 'left', 0xe08a ); ?>
			<?php endif; ?>
		</div>
	...
	public static function ipt_fsqm_save_form_cb() {
		...
		if ( $user_edit ) {
			if ( ! isset( $_POST['ipt_fsqm_user_edit_nonce'] ) || ! wp_verify_nonce( $_POST['ipt_fsqm_user_edit_nonce'], 'ipt_fsqm_user_edit_' . $data_id ) ) {
				$return = array(
					'success' => false,
					'errors' => array(
						0 => array(
		...
		//Check for nonce
		$wpnonce = $_POST['ipt_fsqm_form_data_save'];
		if ( !wp_verify_nonce( $wpnonce, 'ipt_fsqm_form_data_save' ) ) {
			$return = array(
				'success' => false,
				'errors' => array(
					0 => array(
